    ...
    <ul class="flex justify-end items-center container">
      ...
      <span class="block p-4" @click="setColorTheme($colorMode.preference === 'dark' ? 'light' : 'dark')">
        <svg v-if="$colorMode.value === 'dark'" xmlns="http://www.w3.org/2000/svg"
             class="h-6 w-6 text-gray-50 block hover:dark:text-yellow-400 hover:text-yellow-400"
             viewBox="0 0 20 20"
             fill="currentColor">
          <path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z" />
        </svg>
        <svg v-if="$colorMode.value === 'light'" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 block hover:dark:text-yellow-400 hover:text-yellow-400"
